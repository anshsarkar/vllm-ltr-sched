...
echo "=== vllm-ltr 8B LMSYS Final Benchmark Runner (v2) ==="
...
for dataset in "lmsys-Meta-Llama-3-8B-Instruct-t1.0-s0-l8192-c10000-rFalse.jsonl" "PO-gen-lmsys-Meta-Llama-3-8B-Instruct-t1.0-s0-l8192-c10000-rFalse.jsonl"; do
...
for config in \
    ...
    "MODEL/results/opt-125m-llama3-8b-lmsys-class-trainbucket10-b4/usage_config.json"; do
    [ -f "$BENCH_DIR/$config" ] || echo "  WARNING: $config not found (run training first)!"
done

# ---- Run ----
mkdir -p "$PROJECT_ROOT/rep/final_results/benchmarks/results/lmsys_test2"
...
bash "$BENCH_DIR/bench-final-lmsys-v2.sh" 2>&1 | tee "$PROJECT_ROOT/rep/final_results/benchmarks/results/lmsys_test2/bench_run.log"

echo ""
echo "=== LMSYS Final Benchmarks (v2) complete ==="
echo "Results: $PROJECT_ROOT/rep/final_results/benchmarks/results/lmsys_test2/"
