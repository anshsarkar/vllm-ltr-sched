...
LOG_DIR="$PROJECT_ROOT/rep/percentile_8b/logs"
mkdir -p "$LOG_DIR"

NUM_CLASSES=10

echo "=== Training percentile (${NUM_CLASSES} classes) classifiers for 8B ==="
echo "Logs will be saved to $LOG_DIR"

# LMSYS
echo "--- Training LMSYS percentile classifier ---"
python trainer_percentile.py --config configs/config_prefill_opt_classify.txt \
    ...
    --job-dir MODEL \
    --run-id opt-125m-llama3-8b-lmsys-pctl${NUM_CLASSES}-b4 \
    --batch-size 4 \
    --num-classes $NUM_CLASSES \
    --epoch 5 \
    2>&1 | tee "$LOG_DIR/train_lmsys_pctl${NUM_CLASSES}.log"

# ShareGPT
echo "--- Training ShareGPT percentile classifier ---"
python trainer_percentile.py --config configs/config_prefill_opt_classify.txt \
    --file jsonfiles/llama3-8b-sharegpt-train-t1-s0-8192.jsonl \
    --job-dir MODEL \
    --run-id opt-125m-llama3-8b-sharegpt-pctl${NUM_CLASSES}-b4 \
    --batch-size 4 \
    --num-classes $NUM_CLASSES \
    --epoch 5 \
    2>&1 | tee "$LOG_DIR/train_sharegpt_pctl${NUM_CLASSES}.log"

echo "=== Training complete ==="
echo "Logs saved to:"
echo "  $LOG_DIR/train_lmsys_pctl${NUM_CLASSES}.log"
echo "  $LOG_DIR/train_sharegpt_pctl${NUM_CLASSES}.log"
