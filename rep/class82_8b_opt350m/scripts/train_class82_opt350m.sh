...
LOG_DIR="$PROJECT_ROOT/rep/class82_8b_opt350m/logs"
mkdir -p "$LOG_DIR"

echo "=== Training class82 (bucket=100) classifiers for 8B using OPT-350m ==="
echo "Logs will be saved to $LOG_DIR"

# LMSYS
echo "--- Training LMSYS classifier ---"
python trainer.py --config configs/config_prefill_opt_350m_classify.txt \
    --file jsonfiles/lmsys-Meta-Llama-3-8B-Instruct-t1.0-s0-l8192-c20000:30000-rFalse.jsonl \
    --job-dir MODEL \
    --run-id opt-350m-llama3-8b-lmsys-class-trainbucket100-b4 \
    --batch-size 4 \
...
python trainer.py --config configs/config_prefill_opt_350m_classify.txt \
    ...
    --job-dir MODEL \
    --run-id opt-350m-llama3-8b-sharegpt-class-trainbucket100-b4 \
    --batch-size 4 \
    --label-group-size 100 \
    --epoch 5 \
