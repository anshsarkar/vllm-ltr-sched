...
echo "=== vllm-ltr 8B ShareGPT OPT-350m Benchmark Runner ==="
...
SHAREGPT_DATASET="llama3-8b-sharegpt-test-t1-s0-8192.jsonl"
if [ ! -f "$BENCH_DIR/$SHAREGPT_DATASET" ]; then
    echo "  WARNING: $SHAREGPT_DATASET not found!"
fi
...
for config in \
    ...
    "MODEL/results/opt-350m-llama3-8b-sharegpt-class-trainbucket100-b4/usage_config.json"; do
...
bash "$BENCH_DIR/bench-rep-opt350m-sharegpt.sh" 2>&1 | tee "$BENCH_DIR/RESULTS/bench_rep_opt350m_sharegpt_run.log"

echo ""
echo "=== ShareGPT OPT-350m Benchmarks complete ==="
