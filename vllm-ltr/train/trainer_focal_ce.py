...
def parse_args():
    ...
    parser.add_argument("--gamma", type=float, default=2.0,
                        help="Focal loss focusing parameter. gamma=0 reduces to standard CE.")
...
def focal_loss(logits, labels, gamma):
    """Focal loss: -(1 - p_t)^gamma * log(p_t), per-sample then averaged."""
...
def run():
    ...
    args = parse_args()
    print(f"Focal CE with gamma={args.gamma}")
    ...
    for epoch in range(args.epoch):
        ...
        for prompt, labels, origin_len in tqdm(train_dataloader):
            ...
            with torch.autocast(device_type="cuda"):
                ...
                loss = focal_loss(logits, labels.view(-1), args.gamma)
        ...
        with torch.no_grad():
            for prompt, labels, origin_len in tqdm(test_dataloader):
                ...
                labels = labels.to("cuda")
                logits = outputs.view(-1, predictor.model.num_labels)
                test_loss_total += focal_loss(logits, labels.view(-1), args.gamma).item()
