...
def parse_args():
    ...
    parser.add_argument("--pairwise-lambda", type=float, default=1.0,
                        help="Weight of pairwise ranking loss relative to CE.")
...
def pairwise_ranking_loss(expected, labels, margin):
    """Margin-based pairwise ranking loss over all C(B,2) pairs in the batch.

    For each pair (i,j) where labels differ, enforces that the expected
    class index ordering agrees with the true label ordering.
    """
    B = expected.size(0)
    if B < 2:
        return torch.tensor(0.0, device=expected.device)

    idx_i, idx_j = torch.triu_indices(B, B, offset=1, device=expected.device)
    li, lj = labels[idx_i], labels[idx_j]

    # Only keep pairs where labels differ
    mask = li != lj
    if mask.sum() == 0:
        return torch.tensor(0.0, device=expected.device)

    ei = expected[idx_i[mask]]
    ej = expected[idx_j[mask]]
    target = torch.sign((li[mask] - lj[mask]).float())

    return F.margin_ranking_loss(ei, ej, target, margin=margin)
...
def run():
    ...
    args = parse_args()
    print(f"Pairwise CE with lambda={args.pairwise_lambda}, margin={args.pairwise_margin}")
    ...
    class_indices = torch.arange(actual_num_classes, device='cuda', dtype=torch.float32)
    ...
    for epoch in range(args.epoch):
        ...
        for prompt, labels, origin_len in tqdm(train_dataloader):
            ...
            if args.print_loss:
                print(f"loss: {loss.item():.4f}  ce: {ce_loss.item():.4f}  pair: {pair_loss.item():.4f}")
            loss.backward()
        ...
        print(f"Epoch {epoch+1}, Loss: {total_loss/n:.4f}  (CE: {total_ce/n:.4f}, Pair: {total_pair/n:.4f})")
        ...
        with torch.no_grad():
            ...
            print(f"Test Loss: {test_loss_total / len(test_dataloader):.4f}")
